...
import { borrow } from "@/src/api/nulsConnector";
...
const WithdrawDialog: React.FC<WithdrawDialogProps> = ({
  ...
  const transactionUrl = useAppSelector(getBorrowTransaction);
  ...
  const borrowNuls = () => {
    borrow(Number(borrowAmount), address)
      .then((res: any) => {
        dispatch(setBorrowTransaction(res));
  ...
  useEffect(() => {
    setRemainingDebt(
      BigNumber(borrowed)
        .minus(BigNumber(borrowAmount == "" ? BigNumber(0) : borrowAmount))
        .toString()
    );
    setAfterHealthFactor(
      BigNumber(collatreal)
        .multipliedBy(0.95)
        .dividedBy(BigNumber(borrowed).plus(BigNumber(borrowAmount)))
        .toString()
    );
  }, [borrowAmount]);

  useEffect(() => {
    if (afterHealthFactor == "") console.log(afterHealthFactor);
  }, [afterHealthFactor]);
      ...
      <DialogBody className="px-0 pt-0 pb-5">
        ...
        <div className="flex justify-between px-5 mt-5">
          <Typography variant="paragraph" className="text-white">
            Balance: {(Number(supplied) * 0.9).toFixed(2).toString()} NULS
          </Typography>
          <Typography
            variant="paragraph"
            className="underline text-white hover:cursor-pointer"
            onClick={() =>
              setBorrowAmount((Number(borrowed)).toFixed(3).toString())
            }
          >
            MAX
          </Typography>
